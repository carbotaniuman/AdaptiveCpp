...
namespace detail {
  ...
  struct is_valid_generic_integer_scalar : is_in_type_set<
    ...
    // SYCL is really weird about this, so let's support the fixed width
    // integer types too. It seems like `vec` doesn't support the regular
    // This part about the spec does not really seem thought out.
